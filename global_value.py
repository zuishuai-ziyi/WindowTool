...
# 初始化日志文件对象
os.makedirs(api.get_file_path('temp'), exist_ok=True)
log = LogClass(output_to_file=api.get_file_path("temp\\log.log"))

# 读取配置文件
profile_obj = ProfileClass(api.get_file_path("data\\profile\\data.yaml"))
_default_profile = \
{
    'set_up': {
        ...
        'start_choose_window_hotkey': TypeIgnore(  # 启动选择窗口热键
            [
                'ctrl', 'alt', 'd'
            ]
        ),
...
if not profile_obj.check_file():
    ...
    if profile_obj.file_exists():
        os.remove(api.get_file_path("data\\profile\\data.yaml"))
    try:
