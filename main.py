...
class MainWindow(QWidget):
    ...
    def __init__(self) -> None:
        ...
        self.update_sele_wind_timer.timeout.connect(
            self.slot_of_update_selected_window_info
        )
        ...
        self.IsWindow: Callable[[Any], bool] = lambda hwnd: bool(
            isinstance(hwnd, int) and win32gui.IsWindow(hwnd)
        )
        self.IsProcess: Callable[[int], bool] = lambda pid: isinstance(
            pid, int
        ...
        self.hotkey_triggered.connect(
            lambda: self.slot_of_start_get_window_button() and None
        )
    ...
    def main_UI(self) -> None:
        ...
        self.start_get_window_button.clicked.connect(
            self.slot_of_start_get_window_button
        )
        ...
        self.start_get_window_button_from_list.clicked.connect(
            self.slot_of_start_get_window_button_from_list
        )
        ...
        self.sel_wind_info_widgets: dict[
            Literal["window_title"]
            | Literal["window_exe"]
            | Literal["window_pos"]
            | Literal["window_size"]
            | Literal["process_pid"]
            | Literal["process_hwnd"],
            Dict[Any, Any],
        ] = {}
            ...
            "obj": (
                (
                    ...
                    obj.editingFinished.connect(
                        self.slot_of_size_title_pos_input_box_edit_finished
                    ),
                ),
                (obj,),
            )[1],
        ...
        self.sel_wind_info_widgets["window_pos"] = {
            ...
            "obj": (
                (  # 初始化
                    ...
                    x_input_box.editingFinished.connect(
                        self.slot_of_size_title_pos_input_box_edit_finished
                    ),
                    y_input_box.editingFinished.connect(
                        self.slot_of_size_title_pos_input_box_edit_finished
                    ),
                ),
                (x_text, x_input_box, y_text, y_input_box),  # 添加
            )[1],
        }
        self.sel_wind_info_widgets["window_size"] = {
            ...
            "obj": (
                (  # 初始化
                    ...
                    w_input_box.editingFinished.connect(
                        self.slot_of_size_title_pos_input_box_edit_finished
                    ),
                    h_input_box.editingFinished.connect(
                        self.slot_of_size_title_pos_input_box_edit_finished
                    ),
                ),
                (w_text, w_input_box, h_text, h_input_box),  # 添加
            )[1],
        }
        ...
        for k in self.sel_wind_info_widgets:
            ...
            self.selec_window_info_layout.addRow(
                v["display"], t_widget
            )  # 将临时控件添加至布局，以确保宽度固定
        # 将 窗口数据布局 添加至 主布局
        ...
        def _show_message_box(success: bool, code: int | None = None):
            if success and profile_obj["set_up"]["show_info_box"]:
                (
                    ...
                    if profile_obj["set_up"]["show_info_box"]
                    else None
                )
        ...
        dos_buttons: List[
            List[
                QPushButton
                | Callable
                | Dict[Literal["need"], Dict[Literal["pid"] | Literal["hwnd"], bool]]
            ]
        ] = [
            [
                QPushButton("结束所选进程"),
                    ...
                    (
                        ...
                        reason := (
                            ...
                            if kill_success
                                ...
                                "等待进程退出超时"
                                if wait_code == win32con.WAIT_TIMEOUT
                                else "您可能没有适当的权限访问目标文件"
                            )
                        ),
                        (
                            (
            ...
            [
                QPushButton("运行源文件"),
                lambda pid, hwnd: (
                    os.startfile(self.select_obj.exe())
                    if self.select_obj
                    else log("未选中窗口")
                ),
            ],
            [
                QPushButton("源文件所在位置"),
                lambda pid, hwnd: (
                    subprocess.Popen(f'explorer /select, "{self.select_obj.exe()}"')
                    if self.select_obj
                    else log("未选中窗口")
                ),
            ],
            ...
            [
                ...
                lambda: (
                    res := ShowRunDialog(
                        ...
                        None,
                        "运行",
                        "ZiYi 将根据你所输入的名称，为你打开相应的程序、文件夹、文档或 Internet 资源。",
                        0,
                    ),
                    (
                        MessageBox(
                            parent=self,
                            title="错误",
                            top_info="无法定位函数 RunFileDlg_win32 于动态链接库 shell32.dll 中",
                            info="",
                            icon=QMessageBox.Critical,
                        )
                        if not res and profile_obj["set_up"]["show_error_box"]
                        else None
                    ),
                ),
        ...
        if not is_admin():
            dos_buttons.append(
                [
                    ...
                    lambda: (
                        exit_the_app(
                            ExitCode.SUCCESS, reason="提升权限成功，退出当前进程"
                        )
                        if run_again_as_admin(self)
                        else (
                            MessageBox(
                                parent=self,
                                title="错误",
                                top_info="权限提升失败",
                                info="提升权限失败，请手动以管理员身份运行",
                                icon=QMessageBox.Critical,
                            )
                            if profile_obj["set_up"]["show_error_box"]
                            else None
        ...
        for index, item in enumerate(dos_buttons):
            ...
            def make_slot(func, item) -> Callable[[], Any] | Callable[[], None]:
                ...
                def res_func() -> Any | None:
                    ...
                    check_pid_and_solve = lambda pid: (
                        True
                        if self.IsProcess(pid)
                        else profile_obj["set_up"]["show_warning_box"]
                        and MessageBox(
                            parent=self,
                            title="操作失败",
                            top_info=f"未选中有效进程",
                    ...
                    try:
                        ...
                        if len(item) == 3:
                            ...
                            raise TypeError(
                                f"处理 {item} 时发生错误：给定的列表长度为 {len(item)} 应为 2 或 3"
                            )
                    except Exception:
            ...
            dos_button_layout.addWidget(
                item[0], index // max_len_for_1_line, index % max_len_for_1_line
            )
    ...
    def set_window_border(self, hwnd, borderless: bool | None = None) -> None:
        ...
        win32gui.SetWindowPos(hwnd, None, left + 1, top + 1, width + 1, height + 1, 0)
        win32gui.SetWindowPos(hwnd, None, left, top, width, height, 0)
    ...
    def record_window_info(self, hwnd: int):
        ...
        try:
            ...
            for item in infos:
                # 枚举，检查当前窗口是否已记录
                if item["id"]["class_name"] == win32gui.GetClassName(hwnd) or item[
                    "id"
            ...
            has_border = win32gui.GetWindowLong(hwnd, win32con.GWL_STYLE) & (
        ...
        try:
            ...
            for item in infos:
                ...
                if item["id"]["class_name"] == win32gui.GetClassName(hwnd) or item[
                    ...
                    log.debug(
                        f'应用已记录的窗口信息：{item["id"]["class_name"]} - {item["id"]["title"]}'
                    )
                    # 更新窗口信息
                    ...
                    # 设置显示状态
                    win32gui.ShowWindow(
                        hwnd, win32con.SW_SHOW if item["show"] else win32con.SW_HIDE
                    )
                    break
            else:
                log.debug(
                    f"窗口无历史记录：{win32gui.GetClassName(hwnd)} - {win32gui.GetWindowText(hwnd)}"
                )
                # 无记录，不做更改
        except:
    ...
    def chose_window(self) -> None:
        ...
        phandle = ctypes.windll.kernel32.OpenProcess(
            win32con.PROCESS_ALL_ACCESS, False, self.select_pid
        )
        if not phandle:
            # 无法访问目标进程
            log.warning(f"无法访问目标窗口")
            if (
                MessageBox(
                    parent=self,
                    title="拒绝访问",
                    top_info=f"无法访问进程 PID: {self.select_pid}，是否提权？",
                    icon=QMessageBox.Critical,
                    buttons=("是", "否"),
                )
                == "是"
            ):
                if run_again_as_admin(self, f"--hwnd={self.select_hwnd}"):
                    exit_the_app(ExitCode.SUCCESS, reason='提升权限成功，停止当前进程')
                else:
                    if profile_obj["set_up"]["show_warning_box"]:
                        MessageBox(
                            parent=self,
                            title="提权失败",
                            top_info=f"提升权限失败，请检查设置后再试",
                            icon=QMessageBox.Critical,
                        )
                    log.error(f"提权失败")
        ...
        # 监测目标窗口尺寸/位置/标题变化
        self.observe_obj = ObserveWindow(
            self.select_hwnd,  # type: ignore
            lambda old_info, new_info: (
                # 更新输入框文本
                self.sel_wind_info_widgets["window_title"]["obj"][0].setText(
                    new_info["title"]
                ),
                self.sel_wind_info_widgets["window_pos"]["obj"][1].setText(
                    str(new_info["pos"][0])
                ),
                self.sel_wind_info_widgets["window_pos"]["obj"][3].setText(
                    str(new_info["pos"][1])
                ),
                self.sel_wind_info_widgets["window_size"]["obj"][1].setText(
                    str(new_info["size"][0])
                ),
                self.sel_wind_info_widgets["window_size"]["obj"][3].setText(
                    str(new_info["size"][1])
                ),
            ),
            wait_time=0.1,
    ...
    def slot_of_profile_callback(self, op_type: OperationType, data: OperationData):
        ...
        if op_type == OperationType.SET_ITEM and data["key"] == "set_up":
            ...
            self.re_register_start_choose_window_hotkey(
                "+".join(data["old_value"]["start_choose_window_hotkey"])
            )
    ...
    def stop_and_start_timer(
        ...
        time = int(
            profile_obj.get(
                root_key, {profile_key: default_value}, using_callback=False
            ).get(profile_key, default_value)
            * 1000
        )
        if time >= 0:
    ...
    def slot_of_update_on_top(self):
        ...
        if profile_obj.get("set_up", {"on_top_time": -1}).get("on_top_time", -1) >= 0:
            ...
            win32gui.SetWindowPos(
                ...
                win32con.SWP_NOMOVE
                | win32con.SWP_NOSIZE
                | win32con.SWP_NOACTIVATE,  # 置顶时，不移动，不改变大小，不激活窗口（不获取焦点）
            )

    def slot_of_update_keep_work(self):
        """更新保持工作状态"""
        if (
            profile_obj.get("set_up", {"keep_work_time": -1}).get("keep_work_time", -1)
            >= 0
        ):
            # 保持工作
            self.show()
    ...
    def slot_of_size_title_pos_input_box_edit_finished(self) -> None:
        """大小/位置/标题输入框 改变槽函数"""
        # 设置窗口属性
        if self.IsWindow(self.select_hwnd) and not any(
            self.is_setting_input_box[k]
            for k in self.is_setting_input_box
            if k in ("pos", "size")
        ):
            ...
            win32gui.SetWindowPos(  # 设置窗口位置与尺寸
                ...
                # 设置窗口位置
                (
                    int(self.sel_wind_info_widgets["window_pos"]["obj"][1].text())
                    if match_int.match(
                ...
                (
                    int(self.sel_wind_info_widgets["window_pos"]["obj"][3].text())
                    if match_int.match(
                ...
                # 设置窗口尺寸
                (
                    int(self.sel_wind_info_widgets["window_size"]["obj"][1].text())
                    if match_int.match(
                ...
                (
                    int(self.sel_wind_info_widgets["window_size"]["obj"][3].text())
                    if match_int.match(
            ...
            win32gui.SetWindowText(
                self.select_hwnd,
                self.sel_wind_info_widgets["window_title"]["obj"][0].text(),
            )  # 设置标题  # type: ignore
            # 设置后更新输入框，确保数据一致
            self.update_input_box()
    ...
    def update_input_box(self):
        ...
        filepath_text = (
            exe_file_path
            if len(exe_file_path) <= max_show_len
            else exe_file_path[:max_show_len] + "..."
        )
    ...
    def slot_of_start_get_window_button_from_list(self):
        ...
        if self.is_getting_info:
            ...
            self.choos_window_list_window.signal_hwnd.connect(
                self.slot_of_selected_window_from_list
            )
    ...
    def _slot_of_update_selected_window_info(self) -> None:
        ...
        # 获取鼠标当前位置的窗口信息
        ignore_hwnds = [int(main_window.winId())]  # 排除的窗口句柄
        ...
        self.last_window_is_top = bool(
            win32gui.GetWindowLong(self.select_hwnd, win32con.GWL_EXSTYLE)
            & win32con.WS_EX_TOPMOST
        )
    ...
    def closeEvent(self, event: QCloseEvent) -> None:
        ...
        self.stop_get_info()
        # 隐藏/关闭窗口
        if profile_obj.get("set_up", {"show_tray_icon": True}).get(
            "show_tray_icon", True
        ):
            # 显示托盘图标时隐藏窗口
            log("主窗口隐藏")
...
class SetUpWindow(QDialog):
    ...
    def init_ui(self) -> None:
        ...
        self.on_top_time_input_box.textChanged.connect(
            self.slot_of_on_top_time_input_box
        )
        self.set_up_items_layout.addRow(
            QLabel("强制置顶间隔时间(s)"), self.on_top_time_input_box
        )
        ...
        self.set_up_items_layout.addRow(
            QLabel("强制前台间隔时间(s)"), self.keep_work_input_box
        )

        # 添加 开始选择窗口快捷键 按钮
        self.set_start_choose_hotkey_button = QPushButton()
        self.set_start_choose_hotkey_button.setText(
            "+".join(self.set_up_data["start_choose_window_hotkey"])
        )
        ...
        self.set_start_choose_hotkey_button.clicked.connect(
            self.slot_of_set_hotkey_to_start_choose
        )
        self.set_up_items_layout.addRow(
            QLabel("开始选择窗口快捷键"), self.set_start_choose_hotkey_button
        )

        # 添加多选框
        self.check_boxes = {
            "allow_hotkey_start_choose": (
                "是否允许使用快捷键开始选择窗口",
                QCheckBox(),
            ),
            "on_top_with_UIAccess": (
                "是否使用 UIAccess 超级置顶",
                QCheckBox(),
                {"restart": True},
            ),
        ...
        for k, v in self.check_boxes.items():
            ...
            v[1].stateChanged.connect(
                functools.partial(self.slot_of_show_message_boxes, k)
            )
    ...
    def save_change(self):
        """保存设置"""
        # 关闭窗口并保存更改
        self.signal_save.emit({"save": True, "data": self.set_up_data})
    ...
    def closeEvent(self, event: QCloseEvent) -> None:
        ...
        if not self.clicked_ok:
            # 关闭窗口并不保存更改
            self.signal_save.emit({"save": False, "data": None})
        # 关闭窗口
...
class ChooseWindowList(QDialog):
    ...
    def slot_of_update_window_list(self):
        ...
        if (selected_model := self.window_table.selectionModel()) is None:
            ...
            MessageBox(
                parent=self,
                title="错误",
                top_info="尝试更新窗口列表时获取到意外数据:\n表格选择模型 为 None",
                info="单击“确定”以退出",
                icon=QMessageBox.Critical,
            )
            exit_the_app(
                ExitCode.UNKNOWN_ERROR,
                reason="尝试更新窗口列表时获取到意外数据:\n选择模型 为 None",
            )
        else:
    ...
    def slot_of_ok_button(self):
        """确定按钮槽函数"""
        self.signal_hwnd.emit(
            -1 if (self.selected_hwnd is None) else self.selected_hwnd
        )
...
def run_again_as_admin(parent: QWidget | None = None, args="") -> bool:
    """以管理员身份重新运行当前程序 成功时返回 Ture，失败时返回 False"""
    # 请求UAC提权
...
@overload
def exit_the_app(
    code: Literal[ExitCode.SUCCESS], reason: str = "(未提供原因)"
) -> NoReturn:
    """退出程序 | 正常退出"""
    ...
...
@overload
def exit_the_app(
    code: Literal[ExitCode.UNKNOWN_ERROR], reason: str = "(未提供错误)"
) -> NoReturn:
    """退出程序 | 未知错误"""
    ...
...
def exit_the_app(
    code: ExitCode | int = ExitCode.SUCCESS,
    reason: str = "(无原因)",
    restart_args: str = "",
) -> NoReturn:
    """退出程序"""
...
def init() -> argparse.Namespace | None:
    ...
    # 验证命令行参数是否有效
    if not (
        isinstance(command_line_args.hwnd, str) and command_line_args.hwnd.isnumeric()
    ):
        ...
        if is_admin():
            ...
            try:
                ...
                if load_res is None:
                    MessageBox(
                        parent=None,
                        title="错误",
                        top_info="尝试启用 UIAccess 时发生错误，详情请查阅日志",
                        icon=QMessageBox.Critical,
                    )
                    return None
                ...
                if not IsUIA():
                    ...
                    if not StartUIA(
                        exe_path, params, 0, ctypes.byref(pid), get_session_id()
                    ):
                        raise ctypes.WinError(ctypes.get_last_error())
                    else:
                        log(f"UIAccess 启动成功，PID: {pid.value}")
                        exit_the_app(
                            ExitCode.SUCCESS, reason="UIAccess 已启动，停止当前进程"
                        )
                else:
                    log("UIAccess 已启用")
            except Exception:
            ...
            if (
                MessageBox(
                    parent=None,
                    title="提示",
                    top_info="无法启用 UIAccess 置顶：权限不足",
                    info="是否提升权限并启用？",
                    buttons=("提权并启用", "不提权并继续"),
                )
                == "提权并启用"
            ):
                log("尝试提权并重启")
                run_again_as_admin()
                exit_the_app(ExitCode.SUCCESS, reason="提权并重启")
            log("不提权并继续")
...
if __name__ == "__main__":
    try:
        ...
        if command_line_args.hwnd and main_window.IsWindow(select_hwnd := int(command_line_args.hwnd)):  # type: ignore
            ...
            main_window.select_pid = win32process.GetWindowThreadProcessId(
                main_window.select_hwnd
            )[1]
